                  ...
                  <div class="col-md-12">
                      <div class="content-panel">
                          <table class="table table-striped table-advance table-hover">
                            <h4><i class="fa fa-angle-right"></i> Problems</h4>
                            <hr>
                              <thead>
                              <tr>
                                  <th> Problem</th>
                                  <th class="hidden-phone"> Problem Name</th>
                                  <th> Time Limit</th>
                                  <th></th>
                              </tr>
                              </thead>
                              ...
                              <tr>
                                  <td> <a method="post" href="<?php echo site_url('problemshowing/index/'.$problem->problem_id.'/');?>"><?php echo $problem->problem_no; ?> </a></td>
                                  <td><a method="post" href="<?php echo site_url('problemshowing/index/'.$problem->problem_id.'/');?>"><?=$problem->problem_name;?></a></td>
                                  <td> <?php $tl = $problem->time_limit/1000.0; echo $tl.' s';?> </td>
                                  <?php if($this->session->userdata('role')==='Judge'){
                                    ob_start(); //Start output buffer
                                    echo site_url('dashboard/delete/'.$problem->contest_id.'/'.$problem->problem_id.'/');
                          ...
                          <?php if($this->session->userdata('role')==='Judge'){
                            ob_start(); //Start output buffer
                            echo site_url('problemdescription/index/'.$contest_id);
                            $output = ob_get_contents(); //Grab output
                            ob_end_clean();
                            echo '<td><a method="post" href="'.$output.'"><button type="button"  class="btn btn-primary btn-lg">Add New Problem</button></a></td>';
                            /**
                            ob_start(); //Start output buffer
                            echo site_url('dashboard/start_judge/'.$contest_id);
                            $output = ob_get_contents(); //Grab output
                            ob_end_clean();
                            echo '<td><a method="post" href="'.$output.'"><button type="button"  class="btn btn-primary btn-lg">Run Judge</button></a></td>';
                            */
                          }
                          ?>
                      </div><!-- /content-panel -->
