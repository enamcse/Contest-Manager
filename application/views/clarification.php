              ...
              <div class="row mt">
                  <div class="col-md-12">
                      <div class="content-panel">
                          <table class="table table-striped table-advance table-hover">
                            ...
                            <hr>
                              ...
                              <tr>
                                  ...
                                  <th><i class=" fa fa-bullhorn"></i> Asked by </th>
                              ...
                              <tr>
                                  <td><?=$clar->clarification_id;?></td>
                                  <td><a method="post" href="<?php 
                                  if($this->session->userdata('role')==='Judge') echo site_url('clar_show_judge/index/'.$contest_id.'/'.$clar->clarification_id.'/'); 
                                  else echo site_url('clar_show_cont/index/'.$contest_id.'/'.$clar->clarification_id.'/');?>"> <?=$clar->question;?> </a></td>
                                  <td><?=$clar->answer;?></td>
                                  <td><?=$clar->requester?></td>
                                  <td><span class="label 

                                  <?php
                                    $res = $clar->status;
                                    if($res === 'ANSWERED') echo 'label-success';
                                    else echo 'label-primary';
                                   ?>


                                   label-mini"><?=$clar->status;?></span></td>
                                   <?php if($this->session->userdata('role')==='None'){
                                    ob_start(); //Start output buffer
                                    echo site_url('submission/rejudge/'.$contest_id.'/'.$sub->submission_id.'/');
                                    $output = ob_get_contents(); //Grab output
                                    ob_end_clean();
                                    echo '<td><a method="post" href="'.$output.'"><button type="button"  class="btn btn-danger btn-xs">Rejudge</button></a></td>';
                                  }
                                  ?>
                          ...
                          <?php 
                            ob_start(); //Start output buffer
                            echo site_url('clar_ask_cont/index/'.$contest_id);
                            $output = ob_get_contents(); //Grab output
                            ob_end_clean();
                            echo '<td><a method="post" href="'.$output.'"><button type="button"  class="btn btn-primary btn-lg">Ask New Question</button></a></td>';
                          ?>
                      </div><!-- /content-panel -->
                  </div><!-- /col-md-12 -->
              </div><!-- /row -->
